...
light_loop
		LSLS		R6, #0x1
		BCS 		LED_ON_BRANCH		; turn on if the carry is set
		BCC			LED_OFF_BRANCH 		; turn off if the carry out bit is not set
...
LED_ON 	STMFD		R13!,{R3, R14}
		MOV			R3, #0xA0000000 ; flip the 28th bit
		STR 		R3, [R4, #0x20]
		LDMFD		R13!,{R3, R15}
...
;
LED_OFF	STMFD		R13!,{R3, R14}		; preserve R3 and R4 on the R13 stack
		MOV			R3, #0xB0000000 ; flip the 28th bit
		STR 		R3, [R4, #0x20]
		LDMFD		R13!,{R3, R15}
...
DELAY
		MOV 		R7, #0x2C2A 		
		MOVT 		R7, #0x000A 		; Initialize R0 lower word for countdown for 0.5 sec 
	
loop
		SUBS 		R7, #1 			; Decrement r0 and set the N,Z,C status bits
		BNE			loop
...
;
InputLUT	DCB		"AKDCG", 0	; strings must be stored, and read, as BYTES
